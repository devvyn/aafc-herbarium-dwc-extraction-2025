...
def test_dry_run_returns_empty(tmp_path: Path) -> None:
    ...
    text, confidences = gpt_image_to_text.image_to_text(img, model="gpt-4", dry_run=True)
...
def test_success(monkeypatch, tmp_path: Path) -> None:
    ...
    fake_client = SimpleNamespace(responses=SimpleNamespace(create=lambda **kwargs: resp))
...
def test_includes_language_hints(monkeypatch, tmp_path: Path) -> None:
    ...
    text, confidences = gpt_image_to_text.image_to_text(img, model="gpt-4", langs=["eng", "spa"])
