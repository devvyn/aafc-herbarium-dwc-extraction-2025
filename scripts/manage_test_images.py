...
class TestImageManager:
    ...
    def create_sample_bundle(
        self, collection_name: str, output_dir: Path, download: bool = False
    ) -> Dict:
        ...
        for category, count in zip(categories, distribution):
            ...
            category_key = (
                f"{category}_specimens" if not category.endswith("_specimens") else category
            )
    ...
    def validate_urls(self, category: Optional[str] = None) -> Dict[str, Dict]:
        ...
        for cat in categories_to_check:
            ...
            results[cat] = {"total": len(images), "accessible": 0, "errors": []}
...
def main():
    ...
    bundle_parser.add_argument(
        "--output", type=Path, required=True, help="Output directory for bundle"
    )
