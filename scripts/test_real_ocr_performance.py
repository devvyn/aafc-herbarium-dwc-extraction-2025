        ...
        for result in results:
            ...
            for engine_name, engine_result in result.get('engines', {}).items():
                ...
                if 'error' not in engine_result:
                    ...
                    readability_score = engine_result.get('readability', {}).get('readability_score', 0)
    ...
    try:
        if args.command == 'single':
            ...
            if args.summary:
                ...
                print(f"  • Success rate: {summary['successful_tests']}/{summary['total_images_tested']}")

                for engine, perf in summary['engine_performance'].items():
                    if perf['tests_run'] > 0:
                        print(f"  • {engine}: {perf['avg_readability']:.2f} readability, "
