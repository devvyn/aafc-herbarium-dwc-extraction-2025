...
high_quality = [s for s in samples if s['quality_tier'] == 'high_quality' and s['sha256'] not in validated]
...
for i, record in enumerate(high_quality[:5], 1):  # Start with 5
    ...
    # Show extracted data
    print("\n📋 EXTRACTED DATA:")
    for field in ['catalogNumber', 'scientificName', 'eventDate', 'recordedBy', 'locality', 'stateProvince', 'country']:
        value = dwc.get(field, '')
